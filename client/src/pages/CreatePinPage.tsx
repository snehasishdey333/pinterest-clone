...
const CreatePin = () => {
  ...
  const mutation = useMutation({
    ...
    onError: (error) => {
      ...
      alert("There was an error uploading the pin.");
    },
  });
  ...
  const onSubmit = async(data: PinFormValues) => {
    ...
    if (currentUser) {
      formData.append("userId", currentUser.id as string);
    } else {
  ...
  return (
    <div className="max-w-3xl mx-auto p-6">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="flex flex-col items-center border-2 border-dashed border-gray-300 rounded-lg p-6 w-full">
          {imagePreview ? (
        ...
        <div className="flex flex-col space-y-4">
          ...
          <button
            type="submit"
            className="w-full bg-red-500 text-white py-2 rounded-md text-lg hover:bg-red-600"
          >
            Save
          </button>
        </div>
